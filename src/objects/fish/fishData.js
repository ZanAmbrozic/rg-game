const fishData = [
    // LAKE FISH
    {
        name: 'Goldfish',
        type: 'fish',
        modelPath: 'src/objects/fish/goldfish.png',
        sellPrice: 100,
        rarityLevel: 4,
        biome: 'lake',
        caught: false,
    },
    {
        name: 'Trout',
        type: 'fish',
        modelPath: 'src/objects/fish/trout.png',
        sellPrice: 100,
        rarityLevel: 4,
        biome: 'lake',
        caught: false,
    },
    {
        name: 'Largemouth Bass',
        type: 'fish',
        modelPath: 'src/objects/fish/largemouth.png',
        sellPrice: 200,
        rarityLevel: 3,
        biome: 'lake',
        caught: false,
    },
    {
        name: 'Crappie',
        type: 'fish',
        modelPath: 'src/objects/fish/crappie.png',
        sellPrice: 200,
        rarityLevel: 3,
        biome: 'lake',
        caught: false,
    },
    {
        name: 'Gold Fish',
        type: 'fish',
        modelPath: 'src/objects/fish/gold-fish.png',
        sellPrice: 500,
        rarityLevel: 2,
        biome: 'lake',
        caught: false,
    },
    {
        name: 'Diamond Darter',
        type: 'fish',
        modelPath: 'src/objects/fish/diamond-darter.png',
        sellPrice: 500,
        rarityLevel: 1,
        biome: 'lake',
        caught: false,
    },

    // RIVER FISH
    {
        name: 'Chub',
        type: 'fish',
        modelPath: 'src/objects/fish/chub.png',
        sellPrice: 100,
        rarityLevel: 4,
        biome: 'global',
        caught: false,
    },
    {
        name: 'Bullhead',
        type: 'fish',
        modelPath: 'src/objects/fish/bullhead.png',
        sellPrice: 100,
        rarityLevel: 4,
        biome: 'global',
        caught: false,
    },
    {
        name: 'Minnow',
        type: 'fish',
        modelPath: 'src/objects/fish/minnow.png',
        sellPrice: 200,
        rarityLevel: 3,
        biome: 'global',
        caught: false,
    },
    {
        name: 'Stone Loach',
        type: 'fish',
        modelPath: 'src/objects/fish/stone_loach.png',
        sellPrice: 300,
        rarityLevel: 2,
        biome: 'global',
        caught: false,
    },
    {
        name: 'Rainbow Trout',
        type: 'fish',
        modelPath: 'src/objects/fish/rainbow-trout.png',
        sellPrice: 500,
        rarityLevel: 1,
        biome: 'global',
        caught: false,
    },

    // SEA FISH
    {
        name: 'Mackarel',
        type: 'fish',
        modelPath: 'src/objects/fish/mackarel.webp',
        sellPrice: 100,
        rarityLevel: 4,
        biome: 'sea',
        caught: false,
    },
    {
        name: 'Herring',
        type: 'fish',
        modelPath: 'src/objects/fish/herring.png',
        sellPrice: 200,
        rarityLevel: 3,
        biome: 'sea',
        caught: false,
    },
    {
        name: 'Pupfish',
        type: 'fish',
        modelPath: 'src/objects/fish/pupfish.png',
        sellPrice: 400,
        rarityLevel: 2,
        biome: 'sea',
        caught: false,
    },
    {
        name: 'Tigerfish',
        type: 'fish',
        modelPath: 'src/objects/fish/tigerfish.png',
        sellPrice: 800,
        rarityLevel: 1,
        biome: 'sea',
        caught: false,
    },
    {
        name: 'Catfish',
        type: 'fish',
        modelPath: 'src/objects/fish/catfish.gif',
        sellPrice: 1000,
        rarityLevel: 1,
        biome: 'sea',
        caught: false,
    },

    // TRASH
    // {
    //     name: "Boot",
    //     type: "trash",
    //     modelPath: "",
    //     sellPrice: 10,
    //     rarityLevel: 5,
    //     biome: "river",
    //     caught: false,
    // },
    // {
    //     name: "Can",
    //     type: "trash",
    //     modelPath: "",
    //     sellPrice: 5,
    //     rarityLevel: 5,
    //     biome: "sea",
    //     caught: false
    // },
];

export default fishData;

...
export default class Physics2 {
    ...
    resolveCollision(nodeA, nodeB) {
        const meshA = this.getGlobalMesh(nodeA);
        const meshB = this.getGlobalMesh(nodeB);
        ...
        if (mtvAxis && mtvOverlap < Infinity) {
            const transformA = nodeA.getComponentOfType(Transform);
            const transformB = nodeB.getComponentOfType(Transform);
            ...
            const t = player.getComponentOfType(Transform);

            vec3.scaleAndAdd(t.translation, t.translation, mtv, -1);
        }
